-- Create the Snowpipe for VOC_ASMNT_SVC_RLNG_3_MNTH_SG table
CREATE OR REPLACE PIPE VOC_ASMNT_SVC_RLNG_3_MNTH_SG_PIPE
AUTO_INGEST = TRUE
AS
COPY INTO VOC_ASMNT_SVC_RLNG_3_MNTH_SG
(   
    FSCL_YR_NB,
...
FROM (   
    ...
    FROM '@NMEX_MAP_EXT_STG/validated/JDP/SERVICIO VOC 3M'
)
FILE_FORMAT = (   
TYPE = 'CSV',   
FIELD_DELIMITER = ';',   
...
--
select * from VOC_ASMNT_SVC_RLNG_3_MNTH_SG_PIPE;
show pipes;
 
SELECT SYSTEM$PIPE_STATUS('VOC_ASMNT_SVC_RLNG_3_MNTH_SG_PIPE');
ALTER PIPE VOC_ASMNT_SVC_RLNG_3_MNTH_SG_PIPE REFRESH;
SELECT * FROM TABLE (INFORMATION_SCHEMA.COPY_HISTORY('VOC_ASMNT_SVC_RLNG_3_MNTH_SG', START_TIME => DATEADD(hours,-30,CURRENT_TIMESTAMP())));
